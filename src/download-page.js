import React from "react"
import { Navbar} from "react-bootstrap"
import { Nav} from "react-bootstrap"
import { NavItem} from "react-bootstrap"
import { NavDropdown} from "react-bootstrap"
import { Glyphicon } from "react-bootstrap"
import { MenuItem} from "react-bootstrap"
import { FormControl} from "react-bootstrap"
import { FormGroup } from "react-bootstrap"
import { Button } from "react-bootstrap"
import { ButtonToolbar } from "react-bootstrap"
import Slider from "react-slick";
import "./App.scss";
import bg1 from "./bg1.jpg"

// const bg1 = require("./bg1.jpg")

export default class DownloadPage extends React.Component {
  render() {
      const settings = {
        speed: 500,
        rows: 2,
        slidesToShow: 2,
        slidesToScroll: 2
      };
      const settings2 = {
        speed: 500,
        slidesToShow: 3
      };
           
    return (
<div className="RentS-Web">

  <div className="RentS-Im-Box  ">   
    <div class="row">
      <div class="col-sm-12">

        <Navbar inverse collapseOnSelect className="NavStyleBlack">
          <Navbar.Header>
            <Navbar.Brand>
              <img className="navLogo" src="data:image/png;base64,"/>
            </Navbar.Brand>
            <Navbar.Toggle />
          </Navbar.Header>
          <Navbar.Collapse>
          <Nav>
           <NavDropdown className="navText" eventKey={2} title="BUSINESS" id="basic-nav-dropdown">
             <MenuItem eventKey={2.1}>Action</MenuItem>
             <MenuItem eventKey={2.2}>Another action</MenuItem>
             <MenuItem eventKey={2.3}>Something else here</MenuItem>
             <MenuItem divider/>
             <MenuItem eventKey={2.3}>Separated link</MenuItem> 
           </NavDropdown>
           <NavItem className="navText" eventKey={3} href="#">
             BLOG
           </NavItem>
           <NavItem className="navText" eventKey={4} href="#">
             HELP
           </NavItem>
           <NavItem className="navText" eventKey={5} href="#">
             SCREEN TENANTS
           </NavItem>
         </Nav>
         <Nav pullRight>
          <NavItem eventKey={1} href="#">
            <Button className="buttonLogIn" ><b>Log In</b></Button>
          </NavItem>
          <NavItem eventKey={2} href="#">
            <Button className="buttonSignUp" ><b>Sign Up</b></Button>
          </NavItem>
          <NavItem className="navText" eventKey={3} href="#">For Renters</NavItem>
         </Nav>
         </Navbar.Collapse>
        </Navbar>
      </div>
    </div> 
    <div class="row">
      <div class="col-sm-8 col-sm-offset-2 col-xs-offset-1 col-xs-10 text-xs-center boxPanel">
        <h2><b>Verify Tenants at Light-Speed</b></h2>
        <h4>RentSpree will simplify the rental process and land you better tenants.</h4>
        <Button className="RequestButton" ><b>Request a Demo</b>  </Button>
        <h5><i>And I will call you within a few minutes - Michael Lucarelli</i></h5>
      </div> 
    </div> 
  </div> 

  <div className="RentS-Im-Box1"> 
    <div class="row">
      <div class="col-sm-8 col-sm-offset-2 col-xs-offset-1 col-xs-10 text-xs-center">
          <center>
            <div className="box1Header">We make leasing simple.</div>
            <div className="box1Text">Collect online rental applications and screen tenants in one step.</div>
          </center>
      </div>
      </div>
    <div class="row">
      <div class="col-sm-8 col-sm-offset-2 col-xs-offset-1 col-xs-10">
        <div class="col-sm-4 col-xs-12 box-xs-center">
          <img className="imPages" src={bg1}/>
          <div className="box1Topic">1. Request applications</div>
          <div className="box1Detail">Enter your applicants' email addresses on RentSpree. Each renter will instantly receive an email to apply.</div>
        </div> 
        <div class="col-sm-4 col-xs-12 box-xs-center">
          <img className="imPages" src="data:image/png;base64,"/>
          <div className="box1Topic">2. Renters apply</div>
          <div className="box1Detail">Renters will receive a request to apply to your property. They fill out an online rental application and undergo screening.</div>
        </div> 
        <div class="col-sm-4 col-xs-12 box-xs-center">
          <img className="imPages" src="data:image/png;base64,"/>
          <div className="box1Topic">3. View, Print, and Share</div>
          <div className="box1Detail">You can view the completed application and rental history with no additional tenant verification required.</div>
        </div> 
      </div> 
    </div>
  </div> 

  <div className="RentS-Im-Box2"> 
   <div className="box2Triangle"></div>
    <div class="row">
      <div class="col-sm-8 col-sm-offset-2 col-xs-9 col-xs-offset-1">
        <div class="col-sm-6 col-xs-12 ">
          <div className="box2Topic">We built this for you.</div>
          <div className="box2Detail">
            <b>Real Estate Agents</b> 
            — start screening in 60 seconds.Limit your liability and 
            gain instant access to screening reports.
          </div>
          <div className="box2Detail">
            <b>Property Managers</b>
            — receive an online rental application for each unit and 
            allow renters to apply wherever you share the link.
          </div>
          <div className="box2Detail">
            <b>Landlords</b> — screen applicants within seconds using 
            only the renter’s email address. Straightforward interface 
            makes the process painless.Benefits to your applicants
          </div>
          <div className="box2Detail">
          <u><a className="fontWhite">Benefits to your applicants</a></u>
          </div>
        </div>
        <div class="col-sm-6 col-xs-12">
          <div className="box2Topic">What's Included?</div>
          <div className="box2Detail">
          <ul>
            <li>Online Rental Application</li>
            <li>Full TransUnion Credit Report</li>
            <li>Criminal Background Check</li>
            <li>National Eviction Report</li>
          </ul>
          </div>
          <div className="box2Detail">
            <u><a className="fontWhite">Learn more</a></u>
          </div>
            <img className="imPagesB2" src="https://www.rentspree.com/dist/5a6dfea788c4a681cb6dbfc2ee10c980.png"/>
        </div> 
      </div> 
    </div>
  </div> 

  <div className="RentS-Im-Box3"> 
    <div class="row">
      <div class="col-sm-12 text-xs-center">
        <Navbar className="NavStyle">
          <Navbar.Header>
            <Navbar.Text className="navText2">
              <h5>Want to see how RentSpree benefits your business?</h5>
            </Navbar.Text>
          </Navbar.Header>
          <Navbar.Collapse>
            <Navbar.Form pullLeft>
              <Button className="RequestButton ShortButton navCenter" ><b>Request a Demo</b>  </Button>
            </Navbar.Form>
          </Navbar.Collapse>
        </Navbar>
      </div>
      <div class="col-sm-8 col-sm-offset-2">
          <center><h3><b>You're in great company.</b></h3></center><br/>
          <div className="box3Slick">
            <Slider {...settings2}>
            <div>
            <div className="box3Panel">
            "RentSpree helps me get renters to pre-qualify, and it produces superb results."
            </div>
            <div className="box3Triangle"></div> 
            <img className="box3ImgCircle" src="data:image/jpeg;base64,"/>
            </div> 
            <div>
            <div className="box3Panel">
            "Easy to use, and just what I wanted. An application and credit check combined."
            </div> 
            <div className="box3Triangle"></div>
            <img className="box3ImgCircle" src="data:image/jpeg;base64,"/>
            </div>
            <div>
            <div className="box3Panel">
            "It is the best and easy way to check the renters credit and history. I love to use it."
            </div> 
            <div className="box3Triangle"></div>
            <img className="box3ImgCircle" src="data:image/jpeg;base64,"/> 
            </div>
            <div>
            <div className="box3Panel">
            "Very user friendly, It takes maybe 30 seconds and we receive a very easy-to-read report."            
            </div> 
            <div className="box3Triangle"></div>
            <img className="box3ImgCircle" src="data:image/jpeg;base64,"/>
            </div>
            <div>
            <div className="box3Panel">
            "I am now able to deal with four times as many applicants than I would have the old way."            
            </div> 
            <div className="box3Triangle"></div> 
            <img className="box3ImgCircle" src="data:image/jpeg;base64,"/>
            </div>
            <div>
            <div className="box3Panel">
            "I truly like the ease of pulling credit and background history. It's a clean and professional way to deal with rentals!"            
            </div>
            <div className="box3Triangle"></div> 
            <img className="box3ImgCircle" src="data:image/jpeg;base64,"/>
            </div> 
            <div>  
            <div className="box3Panel">
            "Quick, easy, and efficient. Makes for a clean free transaction."            
            </div> 
            <div className="box3Triangle"></div> 
            <img className="box3ImgCircle" src="data:image/jpeg;base64,"/>
            </div>
            <div>
            <div className="box3Panel">
            "An absolute lifesaver when trying to help clients apply for a lease/rental."            
            </div> 
            <div className="box3Triangle"></div> 
            <img className="box3ImgCircle" src="data:image/jpeg;base64,"/>
            </div>
            </Slider>
          </div>
       </div> 
    </div> 
  </div> 
 
  <div className="RentS-Im-Box4"> 
    <div class="row">
      <div class="col-sm-8 col-sm-offset-2 col-xs-offset-1 col-xs-10 "> 
        <div class="col-sm-6 col-xs-12 Set-Padding">     
        <div className="box4Header">Our Clients</div>
        <div className="box4Detail">We have partnered with some of the most trusted names in real estate, including the California Regional Multiple Listings Service, the largest MLS in the U.S., ziplogix™, the largest forms platform in the U.S. and various state and local REALTOR® associations.</div>
        </div> 
        <div class="col-sm-6 col-xs-12">
        <div className="box4Slick">
            <Slider {...settings}>
            <div>
              <img className="imPagesB4" src="https://www.rentspree.com/dist/4cd7dde3c8b5d8e79aa5296a093ac9cd.png"/>
            </div>
            <div>
              <img className="imPagesB4" src="https://www.rentspree.com/dist/8d1f50749db396bd651f34c2d6d64672.png"/>
            </div>
            <div>
              <img className="imPagesB4" src="data:image/png;base64,"/>
            </div>
            <div>
              <img className="imPagesB4" src="https://www.rentspree.com/dist/9a327d091e727ad2609504726fe7d927.png"/>
            </div>
            <div>
              <img className="imPagesB4" src="https://www.rentspree.com/dist/faa21bdccde9aaa232a294978c36f71c.png"/>
            </div>
            <div>
              <img className="imPagesB4" src="https://www.rentspree.com/dist/abd82f14058e247edca89e8d62f770cc.png"/>
            </div>
            <div>
              <img className="imPagesB4" src="data:image/png;base64,"/>
            </div>
            <div>
              <img className="imPagesB4" src="https://www.rentspree.com/dist/b5f50fde7b78c0f047fd90d8c3fe68be.png"/>
            </div>
            <div>
              <img className="imPagesB4" src="https://www.rentspree.com/dist/ab1944c297bdbcdc9078e731f19cb16e.png"/>
            </div>
            <div></div>
            <div>
              <img className="imPagesB4" src="https://www.rentspree.com/dist/ee8d067b47d5ad4057600ac9ae5e50ee.png"/>
            </div>
            <div></div>
            </Slider>
          </div>
        </div>
      </div>
    </div> 
  </div> 

  <div className="RentS-Im-Box5"> 
    <div class="row ">
       <div class="col-sm-8 col-sm-offset-2 col-xs-10 col-xs-offset-1">
          <div class="col-sm-6 col-xs-12 ">
            <img className="imRxT" src="https://www.rentspree.com/dist/c6716c653466fecec93b0a3eb69929a2.png"/>
          </div> 
          <div class="col-sm-6 col-xs-12">
            <div className="box5Header text-xs-center">All-in-one online rental application platform</div>
            <div className="box5Detail">Faster access to reports and reduced liability. RentSpree has teamed up with trusted credit bureau TransUnion to give you a complete application and tenant verification package. Each package includes a full online rental application along with a TransUnion credit report, criminal background check, and national eviction report.</div>
          </div> 
       </div> 
    </div>
  </div> 

  <div className="RentS-Im-Footer"> 
    <div class="row">
      <div class="col-sm-10 col-xs-10 col-sm-offset-1">
        <div class="col-sm-6 col-xs-8 ">
          <img className="Rents-Logo" src="data:image/png;base64,"/> 
          <div className="boxFooterContact">
          <h5 className="boxFooterHeader"><b>ABOUT US </b></h5>
          <a className="boxFooterDetail" href="https://www.rentspree.com/about" target="_blank">About</a><font/>/
          <a className="boxFooterDetail" href="html_images.asp" target="_blank">Press </a><font/>/
          <a className="boxFooterDetail" href="html_images.asp" target="_blank">Help</a><font/>

          <h5 className="boxFooterHeader"><b>RESOURCES </b></h5>
          <a className="boxFooterDetail" href="html_images.asp" target="_blank">Screen Tenants</a><font/>/
          <a className="boxFooterDetail" href="html_images.asp" target="_blank">Blog</a><font/>/            <a className="Inline" href="html_images.asp" target="_blank">Sitemap</a><font/>

          <h5 className="boxFooterHeader"><b>TALK TO US </b></h5>
          <a className="boxFooterDetail" href="html_images.asp" target="_blank">Demo</a><font/>/
          <a className="boxFooterDetail" href="html_images.asp" target="_blank">Contact Us</a><font/>
          </div>
        </div>

        <div class="col-sm-6 col-xs-9">
          <div className="boxFooterSocial">
            <h5 className="boxFooterHeader">CONTACT US</h5>
          </div>
          <div className="boxFooterSocial">
            <div className="boxFooterIcon">
              <i class="fas fa-phone"></i>
            </div>
            <p className="boxFooterDetail">323.515.7757</p>
          </div>
          <div className="boxFooterSocial">
            <div className="boxFooterIcon">
             <i class="far fa-envelope"></i>
            </div>
            <p className="boxFooterDetail">support@rentspree.com</p>
          </div>
          <div className="boxFooterSocial">
            <h5 className="boxFooterHeader">FOLLOW US</h5>
          </div>
          <div className="boxFooterSocial">
            <div className="boxFooterIcon">
              <i class="fab fa-facebook-f"></i>
            </div>
            <div className="boxFooterIcon">
              <i class="fab fa-twitter"></i>
            </div>
            <div className="boxFooterIcon">
              <i class="fab fa-linkedin-in"></i>
            </div>
            <div className="boxFooterIcon">
              <i class="fab fa-instagram"></i>
            </div>
            <div className="boxFooterIcon">
              <i class="fab fa-google"></i>
            </div>
            <div className="boxFooterIcon">
              <i class="fab fa-pinterest-p"></i>
            </div>
            <div className="boxFooterIcon">
              <i class="fab fa-youtube"></i>
            </div>
          </div>
          </div>
        </div> 
    </div>
    <div class="row">
    <div class="col-sm-12 col-xm-10 bottomC">
    <center>
    © All Rights Reserved by RentSpree LLC.Terms of UsePrivacy Policy      (version: 2.1.1)
    </center>
    </div>
    </div>
  </div>         
</div>  
    )
  }
}